    ...
    PanelWindow {
      ...
      Item {
        ...
        Row {
          ...
          Rectangle {
            height: 20
            width: nowPlayingContent.width + 12
            radius: 0
            color: root.theme.bgSurface
          ...
          Rectangle {
            ...
            MouseArea {
              ...
              onWheel: (wheel) => {
                ...
                const current = root.brightnessValue * 100;
                const target = Math.max(5, Math.min(100, current + delta));
                brightnessSetProc.command = ["brightnessctl", "s", Math.round(target) + "%"];
